...
import { useNavigate } from 'react-router-dom'
import { AppContext } from '../context/AppContext'
const RelatedExperts = ({ speciality, expId }) => {
    ...
    useEffect(() => {
        if (experts.length > 0 && speciality) {
            const expertsData = experts.filter((exp) => exp.speciality === speciality && exp._id !== expId)
            setRelExp(expertsData)
        }
    }, [experts, speciality, expId])
